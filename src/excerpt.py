...
@EXCERPT_API.route('/getRandomExcerptIdList', methods=['GET'])
def get_random_excerpt_id_list():
    return _get_random_excerpt_id_list()


@require_token()
def _get_random_excerpt_id_list():
    ...
    return {
        'excerptIdList': [{'speechId': excerpt[0], 'fragment': excerpt[1]} for excerpt in excerpt_list]
    }


@EXCERPT_API.route('/getExcerptListForGame/<game_id>', methods=['GET'])
def get_excerpt_list_for_game(game_id):
    return _get_excerpt_list_for_game(game_id)


@require_token()
def _get_excerpt_list_for_game(game_id):
    with get_db_connection() as connection:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute(
    ...
    return {
        'excerptList': sorted(excerpt_list, key=lambda excerpt: excerpt['counter'])
    }
        ...
        with connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    ''',
                    {'speechId': parameters['speechId'], 'fragment': parameters['fragment']}
                )
    return {}
